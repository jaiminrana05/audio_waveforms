...
class AudioWaveformsInterface {
  ...
  Future<bool> preparePlayer(String path,
      [double? leftVolume, double? rightVolume]) async {
    var result = await _methodChannel.invokeMethod(Constants.preparePlayer, {
      Constants.path: path,
      Constants.leftVolume: leftVolume,
      Constants.rightVolume: rightVolume,
    });
    return result ?? false;
  }
  ...
  Stream<int> listenToDurationStream(){
